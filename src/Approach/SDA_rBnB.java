...
import Approach.BranchAndBound.TreeRbnb;
import Index.AE;
import Index.AEI;
import Index.AEIJ;
import Other.Etc;
...
public class SDA_rBnB extends SubgradientDescentAlgorithm {
    private boolean USE_THREAD = true;

    public SDA_rBnB(Parameter _prmt, Etc _etc) {
        super(_prmt, _etc);
    }
    ...
    public void solve_Routing() {
        if (USE_THREAD) {
            HashMap<AE, TreeRbnb> trees = new HashMap<>();
            ...
            for (int a : prmt.A) {
                ...
                for (int e : aE) {
                    AE kr = new AE(a, e);
                    tree = new TreeRbnb(prmt, a, e, _lm_aek);
            ...
            for (int a : prmt.A) {
                aE = prmt.E_a.get(a);
                for (int e : aE) {
                    TreeRbnb tree = new TreeRbnb(prmt, a, e, _lm_aek);
                    tree.solve();
                    objV_Routing += tree.objV;
                    for (AEIJ key: tree.x_aeij.keySet()) {
                        _x_aeij.put(key, tree.x_aeij.get(key));
